...
// ---- CC (Center Closure): compute μ for each device name and average their |μ| ----
void TableManager::CalculateCCCost() {
    // For each row: flatten the row into unit-cell tokens (same logic as GetTableStringFormat)
    std::unordered_map<std::string, long long> sum_x;   // Σ x for each symbol
    ...
    // Compute μ for each name and average their absolute values |μ|
...
// ---- R (Row Spread): column distance × unit count → per-name mean → population stddev ----
void TableManager::CalculateRCost() {
    std::unordered_map<std::string, double> sum_wdist;  // Σ(dist × count)
...
// ---- Separation: unit-cell based metric using rho(distance), then compute pairwise σ_ij ----
void TableManager::CalculateSpetationCost() {
    ...
    // Compute Σ σ_ij across all pairs
